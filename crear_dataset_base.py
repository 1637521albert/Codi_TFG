...
def add_random_address_from_block(dictionary, mode):
    ...
    with open(f'{data_path}/{mode}/licit_addresses.txt', 'w') as f:
        for address in licit_addresses:
            f.write(address + '\n')
            dictionary[address] = (licit_addresses[address][0], 0)
...
def split_addresses(train_samples, val_samples, test_samples, seed):
    if seed == "licit":
        ...
        for mode, addresses in {'train': train_addresses, 'val': val_addresses, 'test': test_addresses}.items():
            with open(f'{data_path}/{mode}/licit_addresses.txt', 'w') as f:
                for address in addresses:
                    f.write(address + '\n')

    elif seed == "illicit":
        addresses = get_data(prefix + "Data/blocksci_addresses.txt")
        ...
        addresses = get_data(prefix + "Data/blocksci_addresses.txt")
        train_addresses = dict(random.sample(addresses.items(), train_samples // 2))
        remaining_addresses = {k: v for k, v in addresses.items() if k not in train_addresses}
        val_addresses = dict(random.sample(remaining_addresses.items(), val_samples // 2))
        remaining_addresses = {k: v for k, v in remaining_addresses.items() if k not in val_addresses}
        test_addresses = dict(random.sample(remaining_addresses.items(), test_samples // 2))

        add_random_address_from_block(train_addresses, 'train')
        add_random_address_from_block(val_addresses, 'val')
        add_random_address_from_block(test_addresses, 'test')
        ...
        for tx_id, tx_inputs in candidates.items():
            ...
            for out in tx.outputs:
                ...
                if hasattr(addr, 'address_string') and addr.address_string not in addresses:
                    new_addresses[addr.address_string] = (addr, 0)
...
if exp_alg.startswith('fw bw addr whole'):
            ...
            for input_tx in in_tx.inputs:
                ...
                if hasattr(addr, 'address_string'):
                    ...
                    if addr.address_string not in addresses and addr.address_string not in new_addresses:
                        side_addresses[addr.address_string] = (addr, 0)
    ...
    def expand(addresses, num_for_hops, num_back_hops, txs, inputs, outputs, mode=exp_alg.split()[-1], limit_mode=None, limit=math.inf):
        ...
        '''if mode == 'none':
            ...
            print("\nForward expand:")
            for i in range(num_for_hops):
                new, side = expand_forward({**addresses, **news}, new, txs, inputs, outputs, limit_mode=limit_mode, limit=limit)
                news.update(new)
                news.update(side)
            ...
            side = {}
            for i in range(num_back_hops):
                new, side = expand_backward({**addresses, **news}, new, txs, inputs, outputs, limit_mode=limit_mode, limit=limit)
                news.update(new)
                news.update(side)
            ...
            for tx in candidates:
                ...
                for input_tx in tx.inputs:
                    ...
                    if hasattr(input_tx.address, 'address_string'):
                        ...
                        if addr.address_string not in addresses:
                            hop_addresses[addr.address_string] = (addr, 0)
